...
public class Enemy : MonoBehaviour
{
    ...
    private void ManageState()
    {
        ...
        {
            case State.Idle:
                SearchForTargetRunner();
                break;

            case State.Running:
                RunTowardsTargetRunner();
                break;
        }
    }

    private void SearchForTargetRunner()
    {
        ...
        for (int i = 0; i < detectedColliders.Length; i++)
        {
            if (detectedColliders[i].TryGetComponent(out Runner runner))
            {
                ...
                StartRunningTowardsTargetRunner();
                return;
            }
        }
    }

    private void RunTowardsTargetRunner()
    {
    ...
    private void StartRunningTowardsTargetRunner()
    {
        state = State.Running;
